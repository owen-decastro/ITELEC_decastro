# 🛠️ DATA MINING ACTIVITIES & ANALYTICS  
A centralized repository containing all my Data Mining activities, exercises, and exam outputs.  
Feel free to explore each notebook to see the analytical processes, visualizations, and insights generated throughout the course.

---

## 📘 **Activity List**

### 🔹 **Activity 1**
➡️ *Basic Data Mining Concepts & Operations*  
📎 **Notebook:** [ACTIVITY 1](https://github.com/owen-decastro/ITELEC_decastro/blob/main/Activity1%20(1)(1).ipynb)

---

### 🔹 **Activity 2**
➡️ *Data Preprocessing & Cleaning Techniques*  
📎 **Notebook:** [ACTIVITY 2](https://github.com/owen-decastro/PELEC_ACT2/blob/main/Activity2.ipynb)

---

### 🔹 **Midterm Exam**
➡️ *Application of Various Data Mining Methods*  
📎 **Notebook:** [MIDTERM EXAM](https://github.com/owen-decastro/PELEC_repo_of_act/blob/main/midterm_exam.ipynb)

---

### 🔹 **Activity 3 – Dengue PH Analysis**
➡️ *Exploratory Data Analysis (EDA) on Dengue Cases in the Philippines*  
📎 **Notebook:** [ACTIVITY 3](https://github.com/owen-decastro/Activity-3_Dengue_PH/blob/668127963c554a5c1fa6e070e06bf255b039a1da/PROJECT3_DENGUE_PH.ipynb)


---

### 🔹 **Activity 4 – Dengue PH Analysis**
➡️ *Exploratory Data Analysis (EDA) on Dengue Cases in the Philippines*  
📎 **Notebook:** [ACTIVITY 4](https://github.com/owen-decastro/Activity-3_Dengue_PH/blob/668127963c554a5c1fa6e070e06bf255b039a1da/PROJECT3_DENGUE_PH.ipynb)


---

### 🔹 **Assignment 1**
➡️ *Additional Analysis on Dengue Dataset*  
📎 **Notebook:** [ASSIGNMENT 1](https://github.com/owen-decastro/PELEC_repo_of_act/blob/940665990da4ed41c6a2c4903f4e0432b2aa7cfe/PROJECT3_DENGUE_PH.ipynb)

---

## 📊 **About This Repository**
This repository serves as a portfolio of my journey in Data Mining—showcasing my skills in:
- Data cleaning and preprocessing  
- Exploratory data analysis (EDA)  
- Visualization and interpretation  
- Building insights from datasets  
- Python-based data analytics  

---

## ⭐ **Feel free to explore, clone, or fork this repository.**  
If you find this helpful, consider giving it a star!

